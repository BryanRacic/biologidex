...
1. Tree scene continues to work independently
2. Home scene gains tree background without breaking navigation
3. "Lineage Tree" button becomes optional (navigate to dedicated tree scene for advanced features)

### Gradual Rollout
1. **Phase 1-2**: Create components, refactor tree scene (no user-facing changes)
2. **Phase 3**: Integrate into home (user sees tree background)
3. **Phase 4**: Polish and gather feedback

### Rollback Plan
If issues arise, home.tscn can revert to simple UI-only structure by:
1. Removing TreeVisualization instantiation
2. Moving buttons back to CanvasLayer
3. Removing RecenterButton
...
### Component Tests
- [ ] TreeVisualization loads and renders tree correctly
- [ ] TreeVisualization signals emit properly
- [ ] RecenterButton shows/hides based on camera position
- [ ] RecenterButton fade animation works
- [ ] WorldSpaceUI positions children correctly

### Integration Tests
- [ ] Home screen loads with tree background
- [ ] Home buttons remain clickable over tree
- [ ] Pan/zoom works on home screen
- [ ] Recenter button appears when panned
- [ ] Recenter button navigates back to center
- [ ] Tree scene still works independently
- [ ] Navigation between home and other scenes works

### Platform Tests
- [ ] Web export (Chrome, Firefox, Safari)
- [ ] Desktop (Windows, macOS, Linux)
- [ ] Mobile touch (iOS Safari, Android Chrome)

### Performance Tests
- [ ] Home screen maintains 60fps with tree
- [ ] Memory usage acceptable on mobile
- [ ] Lazy loading doesn't cause stutters
